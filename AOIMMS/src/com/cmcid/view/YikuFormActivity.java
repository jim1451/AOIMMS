...
public class YikuFormActivity extends BaseActivity{
	
	private Spinner sp_linliao,sp_kufang,sp_kuwei;
	private Button btn_yiku,btn_cancel,btn_return;
	private String username,linliaoName,kufang,kuwei;
	private ArrayList<HashMap<String, String>> listYiku;
	private ArrayList<HashMap<String, String>> listYiku1;
	...
	private Handler handler = new Handler(){
		public void handleMessage(Message msg) {
			...
			case GET_LINLIAO_NAME:
				...
				sp_linliao.setAdapter(new ArrayAdapter<String>(YikuFormActivity.this, android.R.layout.simple_spinner_item, list1));
				break;
			case GET_KUFANG:
				//下拉显示库房名称
				ArrayList<String> list2 = bundle.getStringArrayList("listKufang");
				sp_kufang.setAdapter(new ArrayAdapter<String>(YikuFormActivity.this, android.R.layout.simple_spinner_item, list2));
				break;
			case GET_KUWEI:
				//下拉显示库位名称
				ArrayList<String> list3 = bundle.getStringArrayList("listKuwei");
				sp_kuwei.setAdapter(new ArrayAdapter<String>(YikuFormActivity.this, android.R.layout.simple_spinner_item, list3));
				break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			setContentView(R.layout.activity_yiku_form);
			getData();
			initViews();
			addListener();
			
			receiver = new YikuReceiver();
			this.registerReceiver(receiver, new IntentFilter(Constant.YIKU_BIZ));
		} catch (Exception e) {
...
//					handler.sendMessage(msg);
	...
	private void initViews() {
		listYiku = (ArrayList<HashMap<String, String>>) getIntent().getSerializableExtra("listYiku");
		listYiku1 = (ArrayList<HashMap<String, String>>) getIntent().getSerializableExtra("listYiku1");
		username = getIntent().getStringExtra("username");
		tv_username = (TextView) findViewById(R.id.tv_yiku_form_username);
		tv_username.setText(username);
		
		tv_tuiku_date = (TextView)findViewById(R.id.tv_yiku_date);
		...
		sp_kufang = (Spinner)findViewById(R.id.sp_yiku_kufang);
		sp_kuwei = (Spinner)findViewById(R.id.sp_yiku_kuwei);
		
		btn_yiku = (Button)findViewById(R.id.btn_yiku_form_yiku);
		btn_cancel = (Button)findViewById(R.id.btn_yiku_form_cancel);
		btn_return = (Button)findViewById(R.id.btn_yiku_form_huanyuan);
	}
	private void addListener() {
		btn_yiku.setOnClickListener(new OnClickListener() {
			...
			//	linliaoName = sp_linliao.getSelectedItem().toString();
				...
				new AlertDialog.Builder(YikuFormActivity.this).setTitle(R.string.yiku)
				.setMessage(R.string.confirm_yiku_message)
				.setPositiveButton(R.string.sure, new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
			//			Tools.showProgressDialog(TuikuFormActivity.this, "正在为您进行退库操作中...");
						Intent intent = new Intent(YikuFormActivity.this,YikuBiz.class);
						intent.putExtra("linliaoName", linliaoName);
						intent.putExtra("kufang", kufang);
						intent.putExtra("kuwei", kuwei);
						intent.putExtra("listYiku", listYiku);
						intent.putExtra("listYiku1", listYiku1);
	...
	class YikuReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(listEPC.size()==0){//全部退库
				new AlertDialog.Builder(context).setMessage(R.string.yiku_done_message)
				...
				.setNegativeButton(R.string.back_to_main, new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						Intent intent = new Intent(YikuFormActivity.this,MainActivity.class);
						intent.putExtra("username", username);
